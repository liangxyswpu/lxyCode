...
def attack_by_krum1(clients_list, server_model_weights, model_name, classes_num, *input_shape):
    bound = 1e-5
    upper = 1
    client_num = len(clients_list)
    ...
    while True:
        ...
        while a > bound:
            ...
            selected_clients_id_list = flvs_check(grads_list)
            # if selected_clients_id < count:
            if count-1 in selected_clients_id_list:
                res = []
                for i in range(len(temp_grads)):
                    res.append(temp_grads[i])
