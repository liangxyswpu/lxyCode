...
def make_non_iid_datasets(X_train, y_train, p: float, clients_num, classes_num):
    ...
    for i in range(len(X_train)):
        pro_list = []  # 分到第几组的概率，如标签为5,则pro为[0.5/9,0.5/9,0.5/9,0.5/9,0.5/9,1/2,0.5/9,0.5/9,0.5/9,0.5/9]
...
def process_scaling_or_labelflipping(client, model_name, classes_num, model_weights, X_train, malicious_label_list, *input_shape):
...
def process_mean(temp_mean, temp_std, temp_direction):
...
def process_gaussian(temp_mean, temp_std):
    temp_grads = np.zeros((len(temp_mean)), dtype=np.float)
    for k in range(len(temp_mean)):  # k表示第i层梯度的第k个梯度
        temp_grads[k] = np.random.normal(temp_mean[k], temp_std[k], 1)[0]
    return temp_grads

def matrices_convert_vectors(w):
    """
    将神经网络的参数矩阵转换为一维向量
    :return:
    """
    vectors = []
    for v in w:
        vectors.extend(v.reshape(-1, ).tolist())
    return vectors


def copy_w_shape_fill_zero(data):
    return copy_w_shape_fill(data, 0)


def copy_w_shape_fill(data, fill_value):
    """
    神经网络参数矩阵复制，并用指定的值进行填充，要保证结果有相同的shape
    :param data:            神经网络参数
    :param fill_value:      填充的值
    :return:
    """
    copy_data = [0] * len(data)
    for index, value in enumerate(data):
        same_shape = np.zeros(value.shape)
        same_shape[same_shape == 0] = fill_value
        copy_data[index] = same_shape
    return copy_data


def cos(array1, array2):
    a_norm = np.linalg.norm(array1)
    b_norm = np.linalg.norm(array2)
    similiarity = np.dot(array1, array2.T) / (a_norm * b_norm)
    # dist = 1. - similiarity
    return similiarity
